...
Users are considered "Moderators" if they have at least one role from the server's configured ["Moderator Roles"](/server-settings/mod-roles).
...
Users are recognized as Administrators if they have at least one role from server's configred ["Administrator Roles"](/server-settings/admin-roles) assigned or have the Discord "Administrator" permission.

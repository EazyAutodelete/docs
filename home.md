---
title: EazyAutodelete
description: With EazyAutodelete you can automatically delete messages. Choose from many different modes, filters and more to customize which messages are deleted.
published: true
date: 2025-11-02T20:49:12.543Z
tags: 
editor: markdown
dateCreated: 2025-11-02T20:09:24.067Z
---

# 🤖 EazyAutodelete Docs

With EazyAutodelete you can automatically delete messages. Choose from many different modes, filters and more to customize which messages are deleted.

You can find a brief explanation on how EazyAutodelete works [here](/getting-started).

## Table of contents

* [🤖 EazyAutodelete](/)
* [➕ Add to server](/add-to-server)
* [🚀 Getting Started](/getting-started)
* [🚮 Delete Configuration](/config)
  * [⚙️ Modes](/config/mode)
  * [⌛ Limit](/config/limit)
  * [🔎 Filters](/config/filters)
  * [🔂 Filter Behavior](/config/filter-behavior)
  * [⏪ Delete messages before](/config/delete-messages-before)
  * [⏩ Delete messages after](/config/delete-messages-after)
  * [👥 Roles](/config/roles)
  * [📡 Load old messages](config/load-old-messages)
* [🌐 Share & copy Configs](/share-and-copy-configs)
* [💬 Reference](/reference)
  * [👑 User Permissions](/reference/user-permissions)
  * [🔙 Load old Messages](/reference/load-old-messages)
* [🔤 Languages](/languages)
* [🛠️ Server Settings](/server-settings)
  * [👑 Administrator Roles](/server-settings/admin-roles)
  * [🛡️ Moderator Roles](/server-settings/mod-roles)
* [✨ Premium](/premium)
* [🤬 Troubleshooting](/troubleshooting)
* [🙌 Voting for EazyAutodelete](/voting-for-eazyautodelete)

### 🔗 Links

* [Bot Invite](https://eazyautodelete.xyz/invite/)
* [Support Server](https://eazyautodelete.xyz/discord/)
* [Top.GG Link](https://top.gg/bot/746453621821931634)
* [Discord Bot List](https://discordbotlist.com/bots/eazyautodelete)
